...
<div class="addressCollumns clear">
    <div class="collumn">
        ...
        <ul class="form clear hide" id="addressForm">
    ...
    <div class="collumn">
        <h3 id="addShippingAddress" class="blockHead">
            ...
            <button id="userChangeShippingAddress"  name="changeShippingAddress" type="submit" class="submitButton largeButton"[{if !$oView->showShipAddress() and $oxcmp_user->getSelectedAddress()}]style="display: none;" [{/if}]>[{ oxmultilang ident="CHANGE" }]</button>
        </h3>
       <p><input type="checkbox" name="blshowshipaddress" id="showShipAddress" [{if !$oView->showShipAddress()}]checked[{/if}] value="0"><label for="showShipAddress">[{ oxmultilang ident="USE_BILLINGADDRESS_FOR_SHIPPINGADDRESS" }]</label></p>
        <ul id="shippingAddress" class="form clear[{if !$oView->showShipAddress()}] hide" [{/if}]>
