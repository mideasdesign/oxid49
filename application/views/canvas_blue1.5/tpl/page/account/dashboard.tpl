...
[{capture append="oxidBlock_content"}]
    <div class="accountDashboardView category-right-content">
        ...
        <div class="col">
            [{block name="account_dashboard_col1"}]
                <dl>
                    <dt><a id="linkAccountPassword" href="[{ oxgetseourl ident=$oViewConf->getSslSelfLink()|cat:"cl=account_password" }]" rel="nofollow">[{ oxmultilang ident="PERSONAL_SETTINGS" }]</a></dt>
                    <dd>[{ oxmultilang ident="CHANGE_PASSWORD" }]</dd>
                </dl>
                <dl>
                ...
                <dl>
                    ...
                    <dd>[{ oxmultilang ident="ORDERS" }] [{ $oView->getOrderCnt() }]</dd>
                </dl>
        ...
        <div class="col">
            [{block name="account_dashboard_col2"}]
                <dl>
                    ...
                    <dd>[{ oxmultilang ident="PRODUCT" }] [{ if $oxcmp_user }][{ $oxcmp_user->getNoticeListArtCnt() }][{else}]0[{/if}]</dd>
                </dl>
                [{if $oViewConf->getShowWishlist()}]
                    <dl>
                        ...
                        <dd>[{ oxmultilang ident="PRODUCT" }] [{ if $oxcmp_user }][{ $oxcmp_user->getWishListArtCnt() }][{else}]0[{/if}]</dd>
                    </dl>
                [{/if}]
                [{if $oViewConf->getShowCompareList()}]
                    <dl>
                        ...
                        <dd>[{ oxmultilang ident="PRODUCT" }] [{ if $oView->getCompareItemsCnt() }][{ $oView->getCompareItemsCnt() }][{else}]0[{/if}]</dd>
                    </dl>
                [{/if}]
                [{if $oViewConf->getShowListmania()}]
                    <dl>
                        <dt><a href="[{ oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=account_recommlist" }]">[{ oxmultilang ident="MY_LISTMANIA" }]</a></dt>
                        <dd>[{ oxmultilang ident="LISTS" }] [{ if $oxcmp_user->getRecommListsCount() }][{ $oxcmp_user->getRecommListsCount() }][{else}]0[{/if}]</dd>
                    </dl>
                [{/if}]
            [{/block}]
        </div>
